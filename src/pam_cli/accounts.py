...
def show_requests(page: Page, timeout_ms: int, keep: list[Page]) -> None:
    """Close any tabs left over from requesting access and show the
    portal's Requests tab. Tabs in `keep` (open before the requests) stay.
    Best-effort: the access was already requested, so a problem here is
    only a warning."""
    for tab in page.context.pages:
        if tab != page and tab not in keep:
            tab.close()
    try:
        page.get_by_role("tab", name="Requests", exact=True).click(timeout=timeout_ms)
    except PlaywrightError as exc:
        logger.warning("Couldn't open the Requests tab: %s", exc)
...
def select_account_and_access(
    ...
    """Request access for each target, print a summary, then show the
    Requests tab. True when done; False if the hours or reason prompt was
    ...
    show_requests(page, timeout_ms, open_tabs)
    return True
